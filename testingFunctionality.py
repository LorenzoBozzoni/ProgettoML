...
#EPOCH_NUMBER = np.linspace(1, 12, 2)    #5
# ten numbers with equal distance from 0 to 50 (5,10,15,20,...)
#BATCH_SIZE = np.linspace(100, 50, 2) #4
...
column_names = ["EpochNumber", "BatchSize", "Accuracy","Loss"]
...
for e_element in EPOCH_NUMBER:
    for b_element in BATCH_SIZE:
        ...
        dfToAppend = pd.DataFrame([[e_element, b_element, np.array(history.history["val_accuracy"]).mean(), np.array(history.history["val_loss"]).mean()]]  , columns=column_names)
        dfToScatter = dfToScatter.append(dfToAppend)
        dfToAppend = dfToAppend[0:0]

fig = plt.figure(figsize=(30,15))
ax = fig.add_subplot(121, projection="3d")
ax.scatter(dfToScatter["EpochNumber"], dfToScatter["BatchSize"], dfToScatter["Accuracy"], edgecolors="red",linewidths=6)
